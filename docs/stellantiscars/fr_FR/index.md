...
Rendez-vous dans `Plugins → Gestion des plugins → Stellantis Cars → Configuration`. Les paramètres
...
Cars`), qui affiche le nombre de véhicules trouvés sur le compte. Le plugin gère ensuite seul le
...
- **Télémétrie** : batterie/état de charge, autonomie (électrique, carburant, totale), position GPS,
  ...
  (échéance de révision), température extérieure, état de l'alarme (si votre véhicule remonte ces
  informations).
...
- **Batterie 12 V** : réveiller un véhicule (manuellement ou via le réveil automatique adaptatif,
  ...
  fragiliser ; laissez la cadence par défaut sauf besoin réel. La valeur remontée par le plugin pour
  cette batterie (commande « Batterie 12 V ») est un **pourcentage** de charge, et non une tension,
  malgré le nom du champ fourni par l'API du constructeur.

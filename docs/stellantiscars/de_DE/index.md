...
Gehen Sie zu `Plugins → Plugin-Verwaltung → Stellantis Cars → Konfiguration`. Die Einstellungen im
...
Cars`) überprüfen, die die Anzahl der im Konto gefundenen Fahrzeuge anzeigt. Das Plugin verwaltet
...
- **Telemetrie**: Batterie/Ladezustand, Reichweite (elektrisch, Kraftstoff, gesamt), GPS-Position,
  ...
  Wartung (Inspektionstermin), Außentemperatur, Alarmstatus (sofern Ihr Fahrzeug diese Informationen
  übermittelt).
...
- **12-V-Batterie**: Das Aufwecken eines Fahrzeugs (manuell oder über das standardmäßig deaktivierte
  ...
  schwächen; belassen Sie die Standardhäufigkeit, sofern kein echter Bedarf besteht. Der vom Plugin
  für diese Batterie gemeldete Wert (Befehl „12-V-Batterie") ist ein **Ladeprozentsatz** und keine
  Spannung, trotz des von der Hersteller-API vergebenen Feldnamens.

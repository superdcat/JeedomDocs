...
Go to `Plugins → Plugin management → Stellantis Cars → Configuration`. The settings in the
...
Cars`), which displays the number of vehicles found on the account. The plugin then manages the
...
- **Telemetry**: battery/charging state, range (electric, fuel, total), GPS position, mileage,
  ...
  outside temperature, alarm status (if your vehicle reports this information).
...
- **12 V battery**: waking up a vehicle (manually or via the adaptive automatic wake-up, disabled by
  ...
  default rate unless you have a real need. The value reported by the plugin for this battery (the
  "12 V battery" command) is a charge **percentage**, not a voltage, despite the field name provided
  by the manufacturer's API.

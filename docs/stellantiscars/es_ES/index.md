...
Vaya a `Plugins → Gestión de plugins → Stellantis Cars → Configuración`. Los parámetros del
...
conectados → Stellantis Cars`), que muestra el número de vehículos encontrados en la cuenta. A
...
- **Telemetría**: batería/estado de carga, autonomía (eléctrica, combustible, total), posición GPS,
  ...
  mantenimiento (fecha de revisión), temperatura exterior, estado de la alarma (si su vehículo
...
- **Batería de 12 V**: despertar un vehículo (manualmente o mediante el despertar automático
  ...
  frecuente puede debilitarla; mantenga la cadencia por defecto salvo necesidad real. El valor que el
  plugin obtiene para esta batería (comando «Batería de 12 V») es un **porcentaje** de carga, y no
  una tensión, a pesar del nombre del campo proporcionado por la API del fabricante.
